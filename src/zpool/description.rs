    ...
    pub fn from_pest_pair(pair: Pair<Rule>) -> Zpool {
        ...
        for pair in pairs {
            match pair.as_rule() {
                ...
                Rule::vdevs => { zpool.vdevs(get_vdevs_from_pair(pair)); }
                Rule::pool_line => { set_stats_and_reason_from_pool_line(pair, &mut zpool); }
...
fn set_stats_and_reason_from_pool_line(pool_line: Pair<Rule>, zpool: &mut ZpoolBuilder) {
    ...
    for pair in pool_line.into_inner() {
        match pair.as_rule() {
            Rule::reason => { zpool.reason(Some(Reason::Other(String::from(pair.into_span().as_str())))); },
            Rule::error_statistics => { zpool.error_statistics(get_error_statistics_from_pair(pair)); },
            _ => {}
        };
    }
}
